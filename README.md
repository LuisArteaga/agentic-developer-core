...
- **CI** — [`.github/workflows/ci.yml`](.github/workflows/ci.yml) calls the [quality-gates-toolkit](https://github.com/LuisArteaga/quality-gates-toolkit) composite (ADR-0059), which runs the secret scan, ruff, mypy, pytest with coverage (≥89%), semgrep, pip-audit, and then the **PR Review Judges**: Syntax/Lint, Test Coverage, Architecture, Security. Judges evaluate per-file chunks with [Enclosing Function Context](CONTEXT.md) (ADR-0022/0023), emit a hidden verdict block (ADR-0019), and produce `PASS` / `FAIL` / `NEEDS REVIEW` verdicts that gate the merge (ADR-0014). [`secret-scan.yml`](.github/workflows/secret-scan.yml) adds gitleaks.
