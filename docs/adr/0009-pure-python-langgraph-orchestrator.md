...
## Context
The previous architecture (outlined in [ADR-0005](./superseded/0005-hybrid-langgraph-orchestrator-with-claude-code-worker.md)) proposed a hybrid orchestrator model where a Python LangGraph process managed the high-level loop (poll, plan, verify, PR, merge) and spawned the Claude Code CLI as a subprocess worker to perform file-editing and local testing. 
