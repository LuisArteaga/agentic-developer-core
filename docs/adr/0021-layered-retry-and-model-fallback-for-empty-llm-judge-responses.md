...
* **Status**: Accepted (amended 2026-08-31 — API-error exhaustion now also triggers the fallback; GLM judges moved to default price-weighted routing; transport retry is 429-aware with an escalating wait budget)
* **Date**: 2026-07-12
...
## Amendment 2026-08-31: 429-Aware Transport Retry Budget

* **Trigger**: The model-level fallback of the first amendment cannot recover from OpenRouter **quota exhaustion**: the fallback model routes through the same API key, so it hits the same 429. The transport retry in `_call_with_api_retry` made exactly 2 attempts 3 seconds apart — a quota window lasts minutes (~45 min observed) — so every real rate-limit episode still died within seconds and surfaced as `NEEDS REVIEW` chunks despite the fallback layer.
* **Transport retry (amended)**: `_call_with_api_retry` replaces the fixed 2-attempt/3 s policy with a retryable-classification + escalating-wait policy. Retryable: HTTP 429, 408, 5xx, network timeouts/connection errors, and in-band API error payloads (preserving the old catch-all). Every other HTTP 4xx (bad request, auth, unknown model) fails fast. Waits follow `(5, 15, 45, 120, 300, 600, 900, 1200)` seconds with ±20% jitter; a server-sent `Retry-After` header overrides the scheduled delay. A total wall-clock budget (`REVIEW_RETRY_BUDGET_SECONDS`, default 2700 s) caps the whole sequence; on exhaustion the error names attempts, elapsed time, and the last error. The layered progression above (primary → nudge → fallback → give up) and the `attempt_count` semantics are unchanged — each progression step simply carries the budgeted transport retry.
* **Debuggability (new)**: `call_openrouter_api` raises a typed `OpenRouterHTTPError` carrying status, `Retry-After`, and a body snippet (OpenRouter encodes rate-limit details in 429 bodies). Every call logs model, attempt, status, and latency; `REVIEW_DEBUG=1` additionally logs payload shape and error bodies. Retry events are appended to `$GITHUB_STEP_SUMMARY` (best effort) so a scrolled-away CI log still shows the retry history; the reusable workflow gains `timeout-minutes: 90` to accommodate the budget.

## Amendment 2026-09-01: Wide-Curated Provider Pin for the GLM Judges

* **Trigger**: Live endpoint data showed the price-weighted default (previous amendment) exposes judge verdicts to quality drift: the cheapest GLM-5.3-flash endpoint (Relace) serves fp4-quantized weights, and degraded endpoints (Venice status -5, GMICloud/Io Net status -2) sit in the default pool. Judge verdicts are the product; provider selection must be deterministic. Routing also cannot address key-level rate limits (probe-sized requests succeed while chunk-sized ones 429) — that remains the transport-retry budget's domain.
* **Decision**: The three GLM judges re-pin from `routing: null` to a wide-curated fp8 order — `["Z.AI", "Novita", "DeepInfra", "Modal", "Fireworks", "Friendli", "Parasail", "Phala"]` — every entry status 0 with ≥98% one-day uptime and ≥131k completion tokens as of 2026-09-01. Eight independent operators make simultaneous pool-wide rate limiting implausible (the failure mode that motivated `null` in the first place), while `allow_fallbacks: false` keeps fp4/degraded endpoints unreachable. The `security` judge is unchanged. Provider rosters drift: re-verify via `GET /api/v1/models/<model-id>/endpoints` before editing any routing list.
