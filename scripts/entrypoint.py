...
def verify_reachability():
    ...
    skip_reachability = os.getenv("SKIP_REACHABILITY", "0").lower() in ("1", "true", "yes")
    ...
    for url in targets:
        ...
        for attempt in range(1, 4):
            ...
            if attempt < 3:
                time.sleep(2)
        if not success:
            log(f"Failed to reach external service {url} after 3 attempts. Exiting.", "ERROR")
            sys.exit(1)
            
    log("All reachability checks passed successfully.")

def run_iteration(consecutive_crashes, run_once, poll_interval, initial_delay=1.0, max_delay=60.0):
    ...
    log(f"Orchestrator process exited with code {exit_code} after {duration:.2f} seconds.")
    ...
    if exit_code == 0:
        ...
        if status == "idle":
            log(f"Agent is idle. Sleeping for poll interval of {poll_interval}s before next run...")
            time.sleep(poll_interval)
        else:
            log("Agent successfully processed a task. Rerunning immediately for next task...")
            time.sleep(1.0) # brief pause to prevent tight CPU looping
    else:
        # Orchestrator crashed
        if duration > 30.0:
            log("Process ran for more than 30s before crashing. Resetting backoff delay to initial value.")
            consecutive_crashes = 0
        ...
        log(f"Orchestrator crash detected (consecutive: {consecutive_crashes}). Sleeping for {total_delay:.2f}s before retry...", "ERROR")
