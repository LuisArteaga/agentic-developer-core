...
class TestTelemetryIntegration(unittest.TestCase):
    ...
    def test_telemetry_with_in_memory_exporter(self):
        ...
        self.assertEqual(loop_span.attributes.get("issue.number"), 456)
        self.assertEqual(loop_span.attributes.get("command.exit_code"), 1)
        
        # Verify attributes on phase spans
        plan_span = spans_by_name["orchestrator_phase_plan"]
        self.assertEqual(plan_span.attributes.get("phase"), "plan")
        self.assertEqual(plan_span.attributes.get("command.exit_code"), 0)
        
        test_writing_span = spans_by_name["orchestrator_phase_test_writing"]
        self.assertEqual(test_writing_span.attributes.get("phase"), "test_writing")
        self.assertEqual(test_writing_span.attributes.get("command.exit_code"), 1)
