...
class TestGitSubprocessHelper(unittest.TestCase):
    def setUp(self):
        ...
        subprocess.run(["git", "init", "-b", "main"], cwd=str(self.repo_path), check=True, capture_output=True)
        ...
        subprocess.run(["git", "add", "initial.txt"], cwd=str(self.repo_path), check=True)
    ...
    def test_current_branch(self):
        ...
        subprocess.run(["git", "checkout", "-b", "feature-test"], cwd=str(self.repo_path), check=True, capture_output=True)
        self.assertEqual(current_branch(self.repo_path), "feature-test")
        
        # Test detached HEAD state
        # Checkout the initial commit directly
        rev_result = subprocess.run(["git", "rev-parse", "HEAD"], cwd=str(self.repo_path), check=True, capture_output=True, text=True)
        commit_hash = rev_result.stdout.strip()
        
        subprocess.run(["git", "checkout", commit_hash], cwd=str(self.repo_path), check=True, capture_output=True)
    ...
    def test_commit_with_and_without_staged_changes(self):
        ...
        result = commit(self.repo_path, "add new file", author="Another Author <another@example.com>")
    ...
    def test_push_success_and_conflict(self):
        ...
        with tempfile.TemporaryDirectory() as remote_temp:
            ...
            subprocess.run(["git", "remote", "add", "origin", str(remote_path)], cwd=str(self.repo_path), check=True)
            ...
            with tempfile.TemporaryDirectory() as clone_temp:
                ...
                subprocess.run(["git", "config", "user.name", "Collaborator"], cwd=str(clone_path), check=True)
                subprocess.run(["git", "config", "user.email", "collab@example.com"], cwd=str(clone_path), check=True)
                ...
                subprocess.run(["git", "add", "initial.txt"], cwd=str(clone_path), check=True)
                subprocess.run(["git", "commit", "-m", "collab commit"], cwd=str(clone_path), check=True)
                subprocess.run(["git", "push", "origin", "main"], cwd=str(clone_path), check=True)
                ...
                subprocess.run(["git", "add", "initial.txt"], cwd=str(self.repo_path), check=True)
                subprocess.run(["git", "commit", "-m", "conflicting commit"], cwd=str(self.repo_path), check=True)
                ...
                self.assertTrue(any(x in ctx.exception.stderr for x in ["rejected", "non-fast-forward", "fetch first"]))
