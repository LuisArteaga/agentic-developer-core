...
- **AGENT_DECISION**: An HTML comment marker (`<!-- AGENT_DECISION: <choice> (<rationale>) -->`) embedded in code changes by the Worker when it makes a design choice not explicitly covered in the specifications.
- **AGENT_RESOLVED**: An HTML comment marker (`<!-- AGENT_RESOLVED: <conflict> (<resolution>) -->`) embedded in code changes by the Worker when it resolves a contradiction between two specifications.
